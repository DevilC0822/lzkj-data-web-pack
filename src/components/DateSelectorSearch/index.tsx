...
import { Select, DatePicker2, Message } from '@alifd/next';
...
import type { IDataSourceValue, IDataSource } from './types';
import './index.scss';
...
const disabledDate = (cur: Dayjs, mode: string, dateType = 'day', dateOffset = -1, customDisabledDate?: any) => {
  if (customDisabledDate) {
    return customDisabledDate(cur, mode);
  }
  return cur > dayjs().add(dateOffset, dateType as ManipulateType).endOf('day');
};
...
const getDefaultDate = (dateType = 'day', dateOffset = -1) => {
  if (dateType === 'mtd' || dateType === 'ytd') {
    return [
      dayjs().add(dateOffset, 'day').startOf(dateType === 'mtd' ? 'month' : 'year').format('YYYY-MM-DD'),
  ...
  if (['last7days', 'last30days', 'R3', 'R6', 'R12'].includes(dateType)) {
    return [
      dayjs().add(dateOffset, 'day').subtract(lastDayMap[dateType], 'day').startOf('day').format('YYYY-MM-DD'),
  ...
  const dealDateType = dateType.includes('range') ? dateType.split('-')[0] : dateType;
  return [
    dayjs().add(dateOffset, dealDateType as ManipulateType).startOf(dealDateType as OpUnitType).format('YYYY-MM-DD'),
    dayjs().add(dateOffset, dealDateType as ManipulateType).endOf(dealDateType as OpUnitType).format('YYYY-MM-DD'),
  ];
};
const dealDataSource = (dataSource: IDataSource[]) => {
  // 如果类型是IDataSourceValue[] 则处理成key value的对象数组
  if (Array.isArray(dataSource) && dataSource.length > 0 && typeof dataSource[0] === 'string') {
    // @ts-ignore
...
const DateSelector: FC<IProps> = (props) => {
  ...
  // 设置value的默认值
  const dealInitValue = () => {
    const dateType = typeof dataSource[0] === 'string' ? dataSource[0] : dataSource[0].value;
  ...
  const onDateChange = (strDate: string) => {
    if (value.dateType === 'mtd' || value.dateType === 'ytd') {
      changeValue({
        ...value,
        startDate: dayjs(strDate).startOf(value.dateType === 'mtd' ? 'month' : 'year').format('YYYY-MM-DD'),
        endDate: strDate,
      });
      return;
    }
    changeValue({
      ...value,
      startDate: strDate,
      endDate: dayjs(strDate).endOf(value.dateType as OpUnitType).format('YYYY-MM-DD'),
    });
  };
  ...
  const onRangeDateChange = (strDates: string[]) => {
    ...
    if (maxTimeInterval && dayjs(strDates[1]).diff(dayjs(strDates[0]), 'day') > maxTimeInterval) {
  ...
  return (
    <div className={`lz-date-selector ${prefix.direction}`}>
      <div className={`prefix-box ${prefix?.customClass}`}>
        {prefix?.name ?? '时间'}{prefix.hasColon ? ':' : ''}
      </div>
      <div style={{ display: 'flex', alignItems: 'center' }}>
        <Select dataSource={dealDataSource(dataSource)} value={value.dateType} onChange={onSelectChange} className={`select ${select?.customClass}`} />
        {
          ['day', 'mtd', 'ytd', 'custom', 'last7days', 'last30days', 'R3', 'R6', 'R12'].includes(value.dateType) && (
            <DatePicker2
              value={value.endDate as string}
              className={`date-picker ${datePicker?.customClass}`}
              onChange={(v) => { onDateChange(v as unknown as string); }}
              outputFormat={'YYYY-MM-DD'}
              disabledDate={(v, mode) => disabledDate(v, mode, 'day', dateOffset, currentItem?.customDisabledDate)}
        ...
        }
        {
          ['week'].includes(value.dateType) && (
            <WeekPicker
              ...
              onChange={(v) => { onDateChange(v as unknown as string); }}
              outputFormat={'YYYY-MM-DD'}
              disabledDate={(v, mode) => disabledDate(v, mode, 'week', dateOffset, currentItem?.customDisabledDate)}
              hasClear={datePicker?.hasClear}
            />
          )
        }
        {
          ['month'].includes(value.dateType) && (
            <MonthPicker
              ...
              onChange={(v) => { onDateChange(v as unknown as string); }}
              outputFormat={'YYYY-MM-DD'}
              disabledDate={(v, mode) => disabledDate(v, mode, 'month', dateOffset, currentItem?.customDisabledDate)}
              hasClear={datePicker?.hasClear}
            />
          )
        }
        {
          ['year'].includes(value.dateType) && (
            <YearPicker
              ...
              onChange={(v) => { onDateChange(v as unknown as string); }}
              outputFormat={'YYYY-MM-DD'}
              disabledDate={(v, mode) => disabledDate(v, mode, 'year', dateOffset, currentItem?.customDisabledDate)}
              hasClear={datePicker?.hasClear}
            />
          )
        }
        {
          ['day-range', 'week-range', 'month-range', 'year-range', 'custom-range', 'P1M', 'P3M', 'P6M', 'P12M'].includes(value.dateType) && (
            <RangePicker
              value={[value.startDate, value.endDate]}
              className={`date-picker ${datePicker?.customClass}`}
              onChange={(v) => { onRangeDateChange(v as unknown as string[]); }}
              outputFormat={'YYYY-MM-DD'}
              mode={(() => {
              ...
              disabledDate={(v, mode) => disabledDate(v, mode, (() => {
                if (['day', 'custom'].includes(value.dateType.split('-')[0])) {
                  return 'day';
                }
                if (['P1M', 'P3M', 'P6M', 'P12M'].includes(value.dateType)) {
                ...
                return value.dateType.split('-')[0] as 'month' | 'year' | 'week';
              })(), dateOffset, currentItem?.customDisabledDate)}
              hasClear={datePicker?.hasClear}
            />
          )
        }
        {
          searchResetGroup.show && <SearchResetGroup {...searchResetGroup} onSearch={onInnerSearch} onReset={onInnerReset} />
        }
      </div>
    </div>
  );
